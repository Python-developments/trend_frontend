...
class CustomAnimatedListState extends State<CustomAnimatedList>with WidgetsBindingObserver {
  ...
  @override
  void initState() {
    toRenderChildren.add(CustomSizedBox(
  ...
  void didUpdateWidget(covariant final CustomAnimatedList oldWidget) {
    final int newElementsLength =
        widget.children.length - (toRenderChildren.length) ~/ 2;
    if (newElementsLength <= 0) {
      return;
    }

    final List<Widget> newElements =
        widget.children.sublist(widget.children.length - newElementsLength);
